...
//duchc
    ...
    public List<ConsignmentDTO> getCanceledProduct(int storeOwnerID, int productStatusID) {
        Connection conn = null;
        ResultSet rs = null;
        PreparedStatement stm = null;
        List<ConsignmentDTO> result = new ArrayList<>();
        ...
        try {
            conn = DBUltilities.makeConnection();
            String query = "SELECT c.*, p.ProductName, p.ProductStatusID FROM Consignment c, Product p WHERE c.StoreOwnerID = ? AND c.ProductID IN "
                    + "(SELECT ProductID FROM Product WHERE ProductStatusID = ? OR ProductStatusID = ?) AND c.ConsignmentStatusID IN (?,?)";
            stm = conn.prepareStatement(query);
            stm.setInt(1, storeOwnerID);
            stm.setInt(2, ProductStatus.NOT_YET_RECEIVE);
            stm.setInt(3, ProductStatus.CANCEL);
            ...
            while (rs.next()) {
                ...
                int productStatus = rs.getInt("ProductStatusID");
                product.setProductStatusID(productStatus);
                String consigmentID = rs.getString("ConsignmentID");
                String cancelDate = formatDateString(rs.getString("CancelDate"));
                item = new ConsignmentDTO();
                item.setConsigmentID(consigmentID);
    ...
    public List<ConsignmentDTO> getAvailableProduct(int storeOwnerID, int productStatusID) {
        Connection conn = null;
        ResultSet rs = null;
        PreparedStatement stm = null;
        List<ConsignmentDTO> result = new ArrayList<>();
        ...
        try {
            conn = DBUltilities.makeConnection();
            String query = "SELECT c.*, p.ProductName, p.ProductID FROM Consignment c, Product p WHERE c.StoreOwnerID = ? AND p.ProductStatusID = ? AND c.ConsignmentStatusID = ? AND c.ProductID = p.ProductID ORDER BY c.ReviewProductDate";
            stm = conn.prepareStatement(query);
            stm.setInt(1, storeOwnerID);
            stm.setInt(2, ProductStatus.AVAILABLE);
    ...
    public List<ConsignmentDTO> getOnWebProduct(int storeOwnerID, int productStatusID) {
        Connection conn = null;
        ResultSet rs = null;
        PreparedStatement stm = null;
        List<ConsignmentDTO> result = new ArrayList<>();
        ...
        try {
            conn = DBUltilities.makeConnection();
            String query = "SELECT c.*, p.ProductName FROM Consignment c, Product p WHERE c.StoreOwnerID = ? AND p.ProductStatusID = ? AND c.ConsignmentStatusID = ? AND p.ProductID = c.ProductID ORDER BY c.RaiseWebDate";
            stm = conn.prepareStatement(query);
            stm.setInt(1, storeOwnerID);
            ...
            while (rs.next()) {
                ...
                product.setName(productName);
                String consigmentID = rs.getString("ConsignmentID");
                String raiseWebDate = formatDateString(rs.getString("RaiseWebDate"));
                item = new ConsignmentDTO();
                item.setConsigmentID(consigmentID);
                item.setRaiseWebDate(raiseWebDate);
                item.setProduct(product);
                result.add(item);
            }
            if (result.isEmpty()) {
    ...
    public List<ConsignmentDTO> getSoldProduct(int storeOwnerID, int productStatusID) {
        Connection conn = null;
        ResultSet rs = null;
        PreparedStatement stm = null;
        List<ConsignmentDTO> result = new ArrayList<>();
        ...
        try {
            conn = DBUltilities.makeConnection();
            String query = "SELECT c.*, p.ProductName, p.SellDate FROM Consignment c, Product p WHERE c.StoreOwnerID = ? AND p.ProductStatusID = ? AND c.ConsignmentStatusID = ? AND p.ProductID = c.ProductID ORDER BY p.SellDate";
            stm = conn.prepareStatement(query);
            stm.setInt(1, storeOwnerID);
            ...
            while (rs.next()) {
                ...
                product.setName(productName);
                String consigmentID = rs.getString("ConsignmentID");
                String sellDate = formatDateString(rs.getString("SellDate"));
                product.setSellDate(sellDate);
                item = new ConsignmentDTO();
                item.setConsigmentID(consigmentID);
                item.setProduct(product);
